...
class AiAgentTaskReminder {
  ...
  constructor() {
    ...
    this.taskOperator = new TaskOperator(this.taskDb, this.esaClient, this.taskOperationAnalyzer);

    // Hooks (イベント・スケジューラ)
    this.schedulerHook = new SchedulerHook(this.taskDb, this.discordClient, this.taskManager);
    ...
    this.checklistInteractionHook = new ChecklistInteractionHook(this.discordClient, this.taskDb);
  ...
  async start(): Promise<void> {
    ...
    if (job) {
      ...
      if (job === 'progress-check') {
        ...
        console.warn(`Unknown job: ${job} (available: progress-check, weekly-tasks)`);
      }
    } else {
    ...
    const autoShutdownMinutes =
      config.autoShutdownMinutes > 0 ? config.autoShutdownMinutes : job ? 10 : 0;
  ...
  private getJobArg(): string | undefined {
    const match = process.argv.map((a) => a.match(/^--job=(.+)$/)).find(Boolean);
    return match?.[1];
  }
